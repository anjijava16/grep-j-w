...
public class MmsPurchaseOrder extends BaseTransUI {
	...
	private TextField tfCstValue, tfSubTaxTotal, tfEDValue, tfHEDValue, tfCessValue, tfVatValue, tfversionNo,
			tfBasictotal, tfPackingValue, tfPONo, tfSubTotal, tfDutyTotal;
	...
	private CheckBox ckdutyexm, ckPdcRqu, ckCformRqu, ckAddress;
	...
	private TextField tfQuoteQnty, tfUnitRate, tfDiscountRate, tfDiscountPer, tfBasicValue, tfPOQty, tfUomTemp;
	...
	private void buildView() {
		...
		ckAddress = new CheckBox("Copy Address");
		ckAddress.addValueChangeListener(new Property.ValueChangeListener() {
			/**
			 * 
			 */
			private static final long serialVersionUID = 1L;
			...
			public void valueChange(ValueChangeEvent event) {
				if (taInvoiceOrd.getValue() != null) {
					taShpnAddr.setValue(taInvoiceOrd.getValue());
				}
			}
		});
		...
		tfQuoteQnty = new TextField();
		tfQuoteQnty.setValue("0");
		tfQuoteQnty.setWidth("100");
		cbMatUom = new ComboBox();
		cbMatUom.setItemCaptionPropertyId("lookupname");
		cbMatUom.setWidth("50");
		cbMatUom.setHeight("20");
		loadUomList();
		tfUnitRate = new TextField("Unit");
		tfUnitRate.setWidth("75");
		...
		tfDiscountPer.addValueChangeListener(new ValueChangeListener() {
			...
			@Override
			public void valueChange(ValueChangeEvent event) {
				if (Long.valueOf((tfUnitRate.getValue())) > Long.valueOf(tfDiscountPer.getValue())
						&& Long.valueOf(tfDiscountPer.getValue()) != 0) {
					calculateDiscount();
				} else {
					if (cbMaterial != null && tfQuoteQnty.getValue() != "0") {
	...
	private void assembleInputUserLayout() {
		...
		flColumn1.addComponent(ckAddress);
		...
		flColumn3.addComponent(tfDutyTotal);
		...
		hlQtyUom.addComponent(tfQuoteQnty);
		...
		hlRate.addComponent(tfUnitRate);
		hlRate.addComponent(tfDiscountRate);
		hlRate.setCaption("Rate");
		flDtlColumn2.addComponent(hlRate);
		flDtlColumn3.addComponent(tfDiscountPer);
		flDtlColumn3.addComponent(tfBasicValue);
		flDtlColumn4.addComponent(taPODtlRemark);
		flDtlColumn5.addComponent(cbPODtlStatus);
	...
	private void loadQuoteDetails() {
		try {
			...
			tfPackingValue.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getPackingValue().toString());
			...
			tfDutyTotal.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getDutyTotal().toString());
			...
			tfHEDPer.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getHedPrcnt().toString());
			tfHEDPer.setReadOnly(true);
			tfHEDValue.setReadOnly(false);
			tfHEDValue.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getHedValue().toString());
			tfHEDValue.setReadOnly(true);
			tfCessPer.setReadOnly(false);
			tfCessPer.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getCessPrcnt().toString());
			tfCessPer.setReadOnly(true);
			tfCessValue.setReadOnly(false);
			tfCessValue.setValue(((MmsQuoteHdrDM) cbQuoteRef.getValue()).getCessValue().toString());
	...
	private void loadMaterialDetails() {
		if (cbMaterial.getValue() != null) {
			...
			tfUnitRate.setValue(((MmsQuoteDtlDM) cbMaterial.getValue()).getUnitrate().toString());
			tfUnitRate.setReadOnly(true);
			tfBasicValue.setReadOnly(false);
			tfBasicValue.setValue(((MmsQuoteDtlDM) cbMaterial.getValue()).getBasicvalue().toString());
	...
	private boolean validatePODetails() {
		...
		if (tfQuoteQnty.getValue() == "0") {
			tfQuoteQnty.setComponentError(new UserError(GERPErrorCodes.SMS_QTY));
			isValid = false;
		} else {
			tfQuoteQnty.setComponentError(null);
		}
	...
	protected void saveDetails() {
		try {
			...
			poHdrDM.setPackingVL(new BigDecimal(tfPackingValue.getValue()));
			poHdrDM.setSubTotal(new BigDecimal(tfSubTotal.getValue()));
			poHdrDM.setSubTotal(new BigDecimal(tfDutyTotal.getValue()));
	...
	private void savePurchaseQuoteDetails() {
		...
		try {
			if (tfPOQty.getValue() != null) {
				...
				tfQuoteQnty.setReadOnly(false);
				poDtlDM.setPoqty((Long.valueOf(tfQuoteQnty.getValue())));
				tfQuoteQnty.setReadOnly(true);
				tfPOQty.setReadOnly(false);
				poDtlDM.setReqQty((Long.valueOf(tfPOQty.getValue())));
				tfPOQty.setReadOnly(false);
				tfUnitRate.setReadOnly(false);
	...
	private void calculateDiscount() {
		...
		try {
			...
			tfDiscountRate.setValue((new BigDecimal(tfUnitRate.getValue())).subtract(
