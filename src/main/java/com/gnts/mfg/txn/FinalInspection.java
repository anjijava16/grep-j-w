...
public class FinalInspection extends BaseTransUI {
	...
	private TextField tfRefNo, tfReportNo, tfBatchQty;
	private ComboBox cbCustCode, cbCaseMod, cbProdDrg, cbCaseCol, cbInsStatus;
	private PopupDateField pdInsdate;
	...
	private void buildView() {
		...
		tfRefNo = new GERPTextField("Inspection No.");
		tfReportNo = new GERPTextField("Product Sl.No");
		tfBatchQty = new GERPTextField("Result");
		cbCustCode = new GERPComboBox("Client Name");
		cbCustCode.setItemCaptionPropertyId("clientName");
		loadClientList();
		cbCaseMod = new GERPComboBox("Product Name");
		cbCaseMod.setItemCaptionPropertyId("prodname");
		loadProductList();
		cbProdDrg = new GERPComboBox("Product Drawing No.");
		cbProdDrg.setItemCaptionPropertyId("drawingCode");
		loadProductDrgCodeList();
		cbCaseCol = new GERPComboBox("Work Order No.");
		cbCaseCol.setItemCaptionPropertyId("workOrdrNo");
		loadWorkOrdNo();
		cbInsStatus = new GERPComboBox("Status", BASEConstants.M_GENERIC_TABLE, BASEConstants.M_GENERIC_COLUMN);
		pdInsdate = new GERPPopupDateField("Inspection Date");
	...
	private void assembleSearchLayout() {
		...
		flTstHdr1.addComponent(tfRefNo);
		flTstHdr2.addComponent(cbCustCode);
		flTstHdr3.addComponent(cbInsStatus);
	...
	private void assembleInputUserLayout() {
		...
		tfRefNo.setRequired(true);
		tfReportNo.setRequired(true);
		tfBatchQty.setRequired(true);
		cbCustCode.setRequired(true);
		cbProdDrg.setRequired(true);
		cbCaseMod.setRequired(true);
		cbCaseCol.setRequired(true);
		cbInsStatus.setRequired(true);
		pdInsdate.setRequired(true);
		...
		flTstHdr1.addComponent(tfRefNo);
		flTstHdr1.addComponent(tfReportNo);
		flTstHdr1.addComponent(tfBatchQty);
		flTstHdr1.addComponent(cbCustCode);
		flTstHdr2.addComponent(cbCaseMod);
		flTstHdr2.addComponent(cbProdDrg);
		flTstHdr2.addComponent(cbProdDrg);
		flTstHdr2.addComponent(cbCaseCol);
		flTstHdr3.addComponent(cbInsStatus);
		flTstHdr3.addComponent(pdInsdate);
	...
	protected void resetFields() {
		...
		tfReportNo.setValue("");
		tfReportNo.setReadOnly(true);
		tfBatchQty.setValue("");
		tfBatchQty.setComponentError(null);
		cbCustCode.setValue("");
		cbProdDrg.setComponentError(null);
		cbCaseMod.setValue(null);
		cbCaseCol.setComponentError(null);
		cbInsStatus.setValue(null);
		pdInsdate.setComponentError(null);
	...
	protected void resetSearchDetails() {
		tfRefNo.setValue(null);
		tfReportNo.setValue(null);
		tfReportNo.setValue("");
		cbInsStatus.setValue(cbInsStatus.getItemIds().iterator().next());
		loadSrchRslt();
	}
	...
	protected void validateDetails() throws ValidationException {
		...
		if (cbCustCode.getValue() == null) {
			cbCustCode.setComponentError(new UserError(GERPErrorCodes.NULL_QATST_CLIENT));
			errorFlag = true;
		} else {
			cbCustCode.setComponentError(null);
		}
		if (cbCaseMod.getValue() == null) {
			cbCaseMod.setComponentError(new UserError(GERPErrorCodes.NULL_QATST_PRODUCT));
			errorFlag = true;
		} else {
			cbCaseMod.setComponentError(null);
		}
		...
		if (cbCaseCol.getValue() == null) {
			cbCaseCol.setComponentError(new UserError(GERPErrorCodes.NULL_QATST_WORKORD_NO));
			errorFlag = true;
		} else {
			cbCaseCol.setComponentError(null);
		}
	...
	protected void saveDetails() throws SaveException, FileNotFoundException, IOException {
		try {
			...
			qaTestHdrDM.setInspectionno(tfBatchQty.getValue());
			qaTestHdrDM.setCompanyid(companyid);
			qaTestHdrDM.setProdslno(tfQADefntstReslt.getValue());
			qaTestHdrDM.setTestresult(tfRefNo.getValue());
			qaTestHdrDM.setClientid((Long) cbCaseCol.getValue());
			qaTestHdrDM.setProductid((Long) cbCustCode.getValue());
			qaTestHdrDM.setProddwgid(Long.valueOf(cbProdDrg.getValue().toString()));
			qaTestHdrDM.setQatestgroupid((Long) cbCustCode.getValue());
	...
	protected void cancelDetails() {
		...
		cbCustCode.setRequired(false);
		cbProdDrg.setRequired(false);
