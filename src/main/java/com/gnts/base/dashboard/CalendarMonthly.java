...
public class CalendarMonthly extends VerticalLayout implements CalendarEventProvider {
	...
	private List<CalendarEvent> getEventsOverlappingForMonthlyTest(Date fromStartDate, Date toEndDate) {
		calendar.setTime(fromStartDate);
		calendar.add(GregorianCalendar.DATE, 5);
		List<CalendarEvent> e = new ArrayList<CalendarEvent>();
		CalendarTestEvent event = getNewEvent("Phase1", fromStartDate, calendar.getTime());
